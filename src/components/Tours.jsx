...
export default function Tours() {
 ...
 const startEditingGuest = (guest) => {
  ...
  editedGuestData.owner =
   editedGuestData.owner === "Yes" || editedGuestData.owner === true;
  editedGuestData.non_owner =
   editedGuestData.non_owner === "Yes" || editedGuestData.non_owner === true;
  setEditedGuest(editedGuestData);
 };

 const editedProjectedPay = ProjectedPayCalculator(
  editedGuest.pay_per_tour,
  editedGuest.gift
 );
 useEffect(() => {
  setEditedGuest((prevGuest) => ({
   ...prevGuest,
   projected_pay: editedProjectedPay,
  }));
 }, [editedProjectedPay]);
 ...
 const updateGuest = async () => {
  ...
  try {
   const response = await fetch(
    `http://localhost:4005/api/Guests/${editGuestId}`,
    {
     method: "PUT",
     headers: {
      "Content-Type": "application/json",
   ...
   if (response.ok) {
    setGuests(
     Guests.map((guest) =>
      guest.guest_id === editGuestId ? updatedEditedGuest : guest
     )
    );
    setEditGuestId(null);
    setEditedGuest({
 ...
 const projectedPay = ProjectedPayCalculator(
  newGuest.pay_per_tour,
  newGuest.gift
 );
 useEffect(() => {
 ...
 const renderGuestForm = (
  ...
  <form className={`custom-form ${formClass}`}>
   <CustomCheckbox
    ...
    onChange={(e) =>
     setGuestFunction({
      ...
      non_owner: !e.target.checked,
     })
    }
   />
   <CustomCheckbox
    label="Non-Owner"
    checked={guest.non_owner}
    onChange={(e) =>
     setGuestFunction({
      ...guest,
      non_owner: e.target.checked,
   ...
   <CustomInput
    ...
    type="text"
    value={guest.name}
    onChange={(e) => setGuestFunction({ ...guest, name: e.target.value })}
   />
   <CustomInput
    placeholder="Pay Per Tour"
    type="smallint"
    value={guest.pay_per_tour}
    onChange={(e) =>
     setGuestFunction({ ...guest, pay_per_tour: e.target.value })
    }
   />
   <CustomSelect
    value={guest.gift}
    options={premiums.map((premiumObj) => premiumObj.premium)}
    onChange={(e) => setGuestFunction({ ...guest, gift: e.target.value })}
   />
   <CustomInput
    placeholder="Projected Pay"
    type="smallint"
    value={guest.projected_pay}
    onChange={(e) =>
     setGuestFunction({ ...guest, projected_pay: e.target.value })
    }
   />
   <CustomInput
    placeholder="Tour Date"
    type="date"
    value={guest.tour_date}
    onChange={(e) => setGuestFunction({ ...guest, tour_date: e.target.value })}
   />
   <CustomInput
    placeholder="Notes"
    type="text"
    value={guest.notes}
    onChange={(e) => setGuestFunction({ ...guest, notes: e.target.value })}
   />
   {buttonText === "Save" ? (
      ...
      <div className="guest-card">
       ...
       <div className="guest-card-body">
        ...
        <div className="guest-detail">
         Projected Pay: ${guest.projected_pay}
        </div>
